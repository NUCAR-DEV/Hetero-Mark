/* THIS TEMPORARY c SOURCE FILE WAS GENERATED BY snack version 0.9.2 */
/* THIS FILE : /home/yifan/hsa/hsaBench/hsa/fir_cl20/fir_Kernels.snackwrap.c  */
/* INPUT FILE: /home/yifan/hsa/hsaBench/hsa/fir_cl20/fir_Kernels.cl  */
/* UPDATED CL: /tmp/snk_8165/updated.cl  */
/*                               */ 
...
hsa_agent_t                      _fir_Kernels_Agent;
hsa_ext_program_t                _fir_Kernels_HsaProgram;
hsa_executable_t                 _fir_Kernels_Executable;
hsa_region_t                     _fir_Kernels_KernargRegion;
int                              _fir_Kernels_FC = 0; 

/* Global variables */
hsa_queue_t*                     Sync_CommandQ;
hsa_signal_t                     Sync_Signal; 
#include "fir_Kernels_brig.h" 

status_t _fir_Kernels_InitContext(){
    ...
    err = hsa_iterate_agents(get_gpu_agent, &_fir_Kernels_Agent);
    if(err == HSA_STATUS_INFO_BREAK) { err = HSA_STATUS_SUCCESS; }
    ErrorCheck(Getting a gpu agent, err);

    /* Query the name of the agent.  */
    char name[64] = { 0 };
    err = hsa_agent_get_info(_fir_Kernels_Agent, HSA_AGENT_INFO_NAME, name);
    ErrorCheck(Querying the agent name, err);
    /* printf("The agent name is %s.\n", name); */

    /* Query the maximum size of the queue.  */
    uint32_t queue_size = 0;
    err = hsa_agent_get_info(_fir_Kernels_Agent, HSA_AGENT_INFO_QUEUE_MAX_SIZE, &queue_size);
    ErrorCheck(Querying the agent maximum queue size, err);
    /* printf("The maximum queue size is %u.\n", (unsigned int) queue_size);  */

    /* Create hsa program.  */
    memset(&_fir_Kernels_HsaProgram,0,sizeof(hsa_ext_program_t));
    err = hsa_ext_program_create(HSA_MACHINE_MODEL_LARGE, HSA_PROFILE_FULL, HSA_DEFAULT_FLOAT_ROUNDING_MODE_DEFAULT, NULL, &_fir_Kernels_HsaProgram);
    ErrorCheck(Create the program, err);

    /* Add the BRIG module to hsa program.  */
    err = hsa_ext_program_add_module(_fir_Kernels_HsaProgram, (hsa_ext_module_t) _fir_Kernels_HSA_BrigMem );
    ErrorCheck(Adding the brig module to the program, err);

    /* Determine the agents ISA.  */
    hsa_isa_t isa;
    err = hsa_agent_get_info(_fir_Kernels_Agent, HSA_AGENT_INFO_ISA, &isa);
    ...
    err = hsa_ext_program_finalize(_fir_Kernels_HsaProgram, isa, 0, control_directives, "", HSA_CODE_OBJECT_TYPE_PROGRAM, &code_object);
    ErrorCheck(Finalizing the program, err);

    /* Destroy the program, it is no longer needed.  */
    err=hsa_ext_program_destroy(_fir_Kernels_HsaProgram);
    ErrorCheck(Destroying the program, err);

    /* Create the empty executable.  */
    err = hsa_executable_create(HSA_PROFILE_FULL, HSA_EXECUTABLE_STATE_UNFROZEN, "", &_fir_Kernels_Executable);
    ErrorCheck(Create the executable, err);

    /* Load the code object.  */
    err = hsa_executable_load_code_object(_fir_Kernels_Executable, _fir_Kernels_Agent, code_object, "");
    ErrorCheck(Loading the code object, err);

    /* Freeze the executable; it can now be queried for symbols.  */
    err = hsa_executable_freeze(_fir_Kernels_Executable, "");
    ErrorCheck(Freeze the executable, err);

    /* Find a memory region that supports kernel arguments.  */
    _fir_Kernels_KernargRegion.handle=(uint64_t)-1;
    hsa_agent_iterate_regions(_fir_Kernels_Agent, get_kernarg_memory_region, &_fir_Kernels_KernargRegion);
    err = (_fir_Kernels_KernargRegion.handle == (uint64_t)-1) ? HSA_STATUS_ERROR : HSA_STATUS_SUCCESS;
    ErrorCheck(Finding a kernarg memory region, err);

    /*  Create a queue using the maximum size.  */
    err = hsa_queue_create(_fir_Kernels_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &Sync_CommandQ);
    ...
    for ( stream_num = 0 ; stream_num < SNK_MAX_STREAMS ; stream_num++){
       ...
       err=hsa_queue_create(_fir_Kernels_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &Stream_CommandQ[stream_num]);
       ErrorCheck(Creating the Stream Command Q, err);
    }

    return STATUS_SUCCESS;
} /* end of _fir_Kernels_InitContext */
...
hsa_executable_symbol_t          FIR_Symbol;
int                              FIR_FK = 0 ; 
status_t                         FIR_init(const int printStats);
status_t                         FIR_stop();
uint64_t                         FIR_Kernel_Object;
uint32_t                         FIR_Kernarg_Segment_Size; /* May not need to be global */
uint32_t                         FIR_Group_Segment_Size;
uint32_t                         FIR_Private_Segment_Size;

/* ------  Start of SNACK function FIR ------ */ 
extern void FIR(float* output,float* coeff,float* temp_input,int  numTap, const snk_lparm_t * lparm) {
   /* Kernel initialization has to be done before kernel arguments are set/inspected */ 
   if (FIR_FK == 0 ) { 
     status_t status = FIR_init(0); 
   ...
   thisKernargAddress = malloc(FIR_Kernarg_Segment_Size); 
   size_t group_base ; 
   group_base  = (size_t) FIR_Group_Segment_Size;
   struct FIR_args_struct {
      uint64_t arg0;
      uint64_t arg1;
      uint64_t arg2;
      uint64_t arg3;
   ...
   struct FIR_args_struct* FIR_args ; 
   /* Setup kernel args */ 
   FIR_args = (struct FIR_args_struct*) thisKernargAddress; 
   FIR_args->arg0=0 ; 
   FIR_args->arg1=0 ; 
   FIR_args->arg2=0 ; 
   FIR_args->arg3=0 ; 
   FIR_args->arg4=0 ; 
   FIR_args->arg5=0 ; 
   FIR_args->arg6 = output ; 
   FIR_args->arg7 = coeff ; 
   FIR_args->arg8 = temp_input ; 
   FIR_args->arg9 = numTap ; 
    ...
    this_aql->kernel_object = FIR_Kernel_Object;
    this_aql->private_segment_size = FIR_Private_Segment_Size;
...
/* ------  End of SNACK function FIR ------ */ 
#include "amd_kernel_code.h"
extern status_t FIR_init(const int printStats){
    if (_fir_Kernels_FC == 0 ) {
       status_t status = _fir_Kernels_InitContext();
       if ( status  != STATUS_SUCCESS ) return; 
       _fir_Kernels_FC = 1;
    }
    ...
    /* printf("Kernel name FIR_: Looking for symbol %s\n","__OpenCL_FIR_kernel"); */
    err = hsa_executable_get_symbol(_fir_Kernels_Executable, NULL, "&__OpenCL_FIR_kernel", _fir_Kernels_Agent , 0, &FIR_Symbol);
    ErrorCheck(Extract the symbol from the executable, err);

    /* Extract dispatch information from the symbol */
    err = hsa_executable_symbol_get_info(FIR_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT, &FIR_Kernel_Object);
    ErrorCheck(Extracting the symbol from the executable, err);
    err = hsa_executable_symbol_get_info(FIR_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE, &FIR_Kernarg_Segment_Size);
    ErrorCheck(Extracting the kernarg segment size from the executable, err);
    err = hsa_executable_symbol_get_info(FIR_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE, &FIR_Group_Segment_Size);
    ErrorCheck(Extracting the group segment size from the executable, err);
    err = hsa_executable_symbol_get_info(FIR_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE, &FIR_Private_Segment_Size);
    ErrorCheck(Extracting the private segment from the executable, err);

    if (printStats == 1) {
       printf("Post-finalization statistics for kernel: FIR \n" );
       amd_kernel_code_t *akc = (amd_kernel_code_t*) FIR_Kernel_Object;
       ...
       printf("%u\n", (uint32_t) FIR_Kernarg_Segment_Size);
    }
