...
* DNN Face Detection Inference - DNN face and facial landmark feature detection is a popular image processing algorithm, key for accurate face recognition. The goal of such algorithms is to identify all possible faces in the image, given no constraints. We use the Multitask Cascaded Convolutional Networks (MTCNN) approach described by Zhang et al.. The algorithm is divided into three pipelined DNN stages.

* Evolutionary Programming - Evolutionary Programming solves optimization problems using an approach that mimics the natural selection process. In our benchmark implementation, we use Evolutionary Programming to solve a non-convex optimization problem.
...
* Force Directed Edge Bundling - Force Directed Edge Bundling is a graph-based data visualization algorithm that helps readers identify patterns in a complex graph. The algorithm models a spring between each pair of the edges and calculates the forces applied to points on each edge. Then each point moves a certain distance towards the direction of the combined force. 

* Gene Alignment - Gene Alignment algorithms are used to answer questions about specific gene sequences (e.g., “CATGCATG”) that occur in the human gene sequence. Our implementation uses a modified version of the Basic Local Alignment Search Tool (BLAST).

* K-Nearest Neighbors - Given a large number of labeled training samples in a multi-dimensional feature space, the K- Nearest Neighbors (KNN) algorithm takes a query point and searches for the K training samples that are close to that point. Using a majority vote approach, the KNN algorithm can categorize the query point with the label that appears the most number of times in the selected K training samples. 
...
* Yifan Sun, Saoni Mukherjee, Trinayan Baruah, Shi Dong, Julian Gutierrez, Prannoy Mohan and David Kaeli, "[Evaluating Performance Tradeoffs on the Radeon Open Compute Platform]()" 2018 IEEE International Symposium
m on Performance Analysis of Systems and Software (ISPASS), Belfast, 2018.
