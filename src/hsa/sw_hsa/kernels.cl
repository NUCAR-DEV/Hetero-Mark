__kernel void sw_init_psi_p(const double a, const double di, const double dj,
                            const double pcf, const int M_LEN, const int N_LEN,
...
__kernel void sw_init_velocities(const double dx, const double dy, const int M,
                                 const int N, __global const double *psi,
  ...
  if (x < N && y < M) {
    u[(y + 1) * M + x] =
        -(psi[(y + 1) * M + x + 1] - psi[(y + 1) * M + x]) / dy;
    v[y * M + x + 1] = (psi[(y + 1) * M + x + 1] - psi[y * M + x + 1]) / dx;
  }
}

__kernel void sw_compute0(const double fsdx, const double fsdy, const int M_LEN,

                          __global const double *u, __global const double *v,
  ...
  cu[(y + 1) * M_LEN + x] = 0.5 * (p[(y + 1) * M_LEN + x] + p[y * M_LEN + x]) *
                            u[(y + 1) * M_LEN + x];
  cv[y * M_LEN + x + 1] =
      0.5 * (p[y * M_LEN + x + 1] + p[y * M_LEN + x]) * v[y * M_LEN + x + 1];
  z[(y + 1) * M_LEN + x + 1] =
      (fsdx * (v[(y + 1) * M_LEN + x + 1] - v[y * M_LEN + x + 1]) -
       fsdy * (u[(y + 1) * M_LEN + x + 1] - u[(y + 1) * M_LEN + x])) /
      (p[y * M_LEN + x] + p[(y + 1) * M_LEN + x] + p[(y + 1) * M_LEN + x + 1] +
       p[y * M_LEN + x + 1]);
  h[y * M_LEN + x] = p[y * M_LEN + x] +
                     0.25 * (u[(y + 1) * M_LEN + x] * u[(y + 1) * M_LEN + x] +
                             u[y * M_LEN + x] * u[y * M_LEN + x] +
                             v[y * M_LEN + x + 1] * v[y * M_LEN + x + 1] +
                             v[y * M_LEN + x] * v[y * M_LEN + x]);
}

__kernel void sw_update0(const int M, const int N, const int M_LEN,
                         __global double *cu, __global double *cv,
                         __global double *z, __global double *h) {
  int x = get_global_id(0);
  int y = get_global_id(1);
...
__kernel void sw_compute1(const double tdts8, const double tdtsdx,
                          const double tdtsdy, const int M_LEN,
                          ...
                          __global double *u_next, __global double *v_next,
  ...
  u_next[(y + 1) * M_LEN + x] =
      u_curr[(y + 1) * M_LEN + x] +
      tdts8 * (z[(y + 1) * M_LEN + x + 1] + z[(y + 1) * M_LEN + x]) *
          (cv[(y + 1) * M_LEN + x + 1] + cv[y * M_LEN + x + 1] +
           cv[y * M_LEN + x] + cv[(y + 1) * M_LEN + x]) -
      tdtsdx * (h[(y + 1) * M_LEN + x] - h[y * M_LEN + x]);

  v_next[y * M_LEN + x + 1] =
      v_curr[y * M_LEN + x + 1] -
      tdts8 * (z[(y + 1) * M_LEN + x + 1] + z[y * M_LEN + x + 1]) *
          (cu[(y + 1) * M_LEN + x + 1] + cu[y * M_LEN + x + 1] +
           cu[y * M_LEN + x] + cu[y * M_LEN + x + 1]) -
      tdtsdy * (h[y * M_LEN + x + 1] - h[y * M_LEN + x]);

  p_next[y * M_LEN + x] =
      p_curr[y * M_LEN + x] -
      tdtsdx * (cu[(y + 1) * M_LEN + x] - cu[y * M_LEN + x]) -
      tdtsdy * (cv[y * M_LEN + x + 1] - cv[y * M_LEN + x]);
}

__kernel void sw_update1(const int M, const int N, const int M_LEN,
                         __global double *u_next, __global double *v_next,
                         __global double *p_next) {
  int x = get_global_id(0);
  int y = get_global_id(1);
...
__kernel void sw_time_smooth(const int M, const int N, const int M_LEN,
                             const double alpha, __global double *u,
                             __global double *v, __global double *p,
                             __global double *u_curr, __global double *v_curr,
                             __global double *p_curr, __global double *u_next,
  ...
  u_curr[y * M_LEN + x] =
      u[y * M_LEN] +
      alpha * (u_next[y * M_LEN + x] - 2. * u[y * M_LEN + x] +
               u_curr[y * M_LEN + x]);
  v_curr[y * M_LEN + x] =
      v[y * M_LEN] +
      alpha * (v_next[y * M_LEN + x] - 2. * v[y * M_LEN + x] +
               v_curr[y * M_LEN + x]);
  p_curr[y * M_LEN + x] =
      p[y * M_LEN] +
      alpha * (p_next[y * M_LEN + x] - 2. * p[y * M_LEN + x] +
               p_curr[y * M_LEN + x]);
}
