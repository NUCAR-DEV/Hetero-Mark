...
void KMEANS::Kmeans_ocl_svm()
{
	...
	clSetKernelArgSVMPointer(kernel_s, 0, (void*) &feature_swap_svm);
	clSetKernelArgSVMPointer(kernel_s, 1, (void*) &cluster_svm);
	clSetKernelArgSVMPointer(kernel_s, 2, (void*) &membership_svm);
	clSetKernelArg(kernel_s, 3, sizeof(cl_int), (void*) &npoints);
	clSetKernelArg(kernel_s, 4, sizeof(cl_int), (void*) &nclusters);
	clSetKernelArg(kernel_s, 5, sizeof(cl_int), (void*) &nfeatures);
	clSetKernelArg(kernel_s, 6, sizeof(cl_int), (void*) &offset);
	...
	for (i = 0; i < npoints; i++)
	{
		...
		if (membership_OCL[i] != membership_svm[i])
		{
			// update membership
			delta_tmp++;
			membership_svm[i] = membership_OCL[i];
		}
