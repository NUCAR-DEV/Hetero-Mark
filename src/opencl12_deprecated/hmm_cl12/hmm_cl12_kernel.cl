...
//                                       Forward kernels
//---------------------------------------------------------------------------//
__kernel void FWD_init_alpha(const int N, __global float *b,
                             __global float *prior, __global float *alpha,
                             __global float *beta) {
...
__kernel void FWD_norm_alpha(const int N, const int startpos, __local float *sm,
                             __global float *alpha, __global float *lll)
...
__kernel void TransposeSym(const int N, __local float *sm, __global float *a,
                           __global float *aT) {
  // x : col			y: row
...
__kernel void FWD_update_alpha(const int N, const int current,
                               __local float *sm, __constant float *constMem,
                               __global float *aT, __global float *b,
...
//                                         Backward kernels
//---------------------------------------------------------------------------//
__kernel void BK_BetaB(const int N, const int pos, __global const float *beta,
                       __global const float *b, __global float *betaB) {
  size_t idx = get_global_id(0);
  if (idx < N) {
...
__kernel void BK_update_beta(const int N, const int current, __local float *sm,
                             __constant float *constMem, __global float *a,
                             __global float *beta) {
  // col
  size_t lx = get_local_id(0);
  size_t gx = get_global_id(0);
...
__kernel void BK_norm_beta(const int N, const int current, __local float *sm,
                           __global float *beta)
...
__kernel void EM_betaB_alphabeta(const int N, const int current,
                                 const int previous, __global const float *beta,
...
__kernel void EM_update_gamma(const int N, const int current, __local float *sm,
...
// Compute alpha * betaB'
__kernel void EM_alpha_betaB(const int N, const int current,
...
__kernel void EM_pre_xisum(const int N, __local float *sm,
  ...
  if (lx == 0 && ly == 0) {
    ...
    blk_result[index_out] = sm[0] + sm[17] + sm[34] + sm[51] + sm[68] + sm[85] +
                            sm[102] + sm[119] + sm[136] + sm[153] + sm[170] +
                            sm[187] + sm[204] + sm[221] + sm[238] + sm[255];
  }
}

__kernel void EM_update_xisum(const int N, const float sum,
...
// TODO
__kernel void EM_gamma(const int N, const int current, __local float *sm,
...
__kernel void EM_expectA(const int N, __local float *sm,
...
__kernel void EM_gamma_state_sum(const int N, const int T, __local float *sm,
...
__kernel void EM_gamma_obs(const int D, const int T, __constant float *constMem,
  ...
  if (gx < T && gy < D) gamma_obs[id] = observations[id] * constMem[gx];
}

__kernel void EM_expect_mu(const int D, const int T, const int offset,
                           const int hs, __local float *sm,
  ...
  if (gx == 0) {  // only 16 threads are alive now
    ...
    if (gy < D) expect_mu[offset + gy] = data / gamma_state_sum[hs];
  }
}

__kernel void EM_sigma_dev(const int D, const int T, const int hs,
...
#pragma unroll
  ...
  sigma_dev[gy * D + gx] =
      sum / gamma_state_sum[hs] - constMem[gy] * constMem[gx];
}

__kernel void EM_expect_sigma(const int blk_rows, const int width,
                              const int start, __global const float *sigma_dev,
