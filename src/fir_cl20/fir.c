...
int main(int argc , char** argv) {
  ...
  while( count < numBlocks )
    {	
      ...
      cl_ulong c_start = 0, c_end = 0;
      clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &c_start, NULL);
      clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &c_end, NULL);
      double time = (cl_double)((c_end - c_start)/1e-06); 
      execTimeMs += time;
